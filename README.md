...
For automatic market suggestions, the optimiser treats the start of the displayed low-price band as the target: the **earliest fully fitting wash start wins**. It compares timer values that can be selected now with minute-aligned moments at which the machine could be activated later. Waiting briefly is recommended when that lets a fixed timer step start the wash earlier—for example, exactly when the low-price band begins. The raw wholesale average is only a tie-breaker between schedules with the same start time. Only when no complete fit exists at all does the optimiser fall back to the candidate with the greatest overlap, preferring the earliest start before using price as the next tie-breaker. This priority is shared by every machine profile and applies to both start- and finish-timer controls. A manually entered cheap-energy window keeps the normal calculator behaviour, including its centred complete-fit suggestion. For today, the app also shows the latest safe-start time that keeps the full cycle and selected safety margin inside the low-price band. The safety margin defaults to **0 minutes**; 10-, 15-, and 30-minute margins remain available under **Fine-tune**.
