...
For market suggestions, the optimiser first checks every timer value that can be selected **now**. If any of them keeps the complete cycle inside the displayed low-price band and selected safety margin, the cheapest of those current settings wins—even if waiting could reduce the raw wholesale average slightly. Waiting to activate the machine is considered only when no current setting gives a complete fit. Only when no complete fit exists at all does the optimiser fall back to the candidate with the greatest overlap, using price as the tie-breaker. This priority is shared by every machine profile and applies to both start- and finish-timer controls. For today, the app also shows the latest safe-start time that keeps the full cycle and selected safety margin inside the low-price band. The safety margin defaults to **0 minutes**; 10-, 15-, and 30-minute margins remain available under **Fine-tune**.
